...
export class PersonalInfoFormComponent implements OnInit {
  ...
  addValidatorControl() {
    this.formDirective?.form
      .get('personalInfoFormGroup.raisonSociale')
    ...
    this.formDirective?.form
      .get('personalInfoFormGroup.sirenSiret')
      ?.addValidators([Validators.required, Validators.pattern(/^[0-9]{9,14}$/)]);
    this.formDirective?.form.get('personalInfoFormGroup.sirenSiret')?.updateValueAndValidity();

    this.formDirective?.form
      .get('personalInfoFormGroup.formeJuridique')
      ?.addValidators([Validators.required, Validators.pattern(/^[a-zA-ZÀ-ÖØ-öø-ÿ]+([ '-][a-zA-ZÀ-ÖØ-öø-ÿ]+)*$/)]);
